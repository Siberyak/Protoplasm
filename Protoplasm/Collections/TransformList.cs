...
namespace Protoplasm.Collections
{
	...
	{
        protected TransformList(Func<TBase, TResult> fromBaseToResult, Func<TResult, TBase> fromResultToBase, bool processIBindingListItem = true)
            : this(GetDataList(processIBindingListItem), fromBaseToResult, fromResultToBase, processIBindingListItem)
		{
		}

        private static IList<TBase> GetDataList(bool processIBindingListItem = true)
	    {
	        return new DataList<TBase>(processIBindingListItem);
	    }

        protected TransformList(IList<TBase> baseList, Func<TBase, TResult> fromBaseToResult, Func<TResult, TBase> fromResultToBase, bool processIBindingListItem = true)
            : base(baseList, fromBaseToResult, fromResultToBase, processIBindingListItem)
		{
		}
